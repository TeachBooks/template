...
13. For colours with hyphens we use need to use the class-based command `\class{named-colour}{...}. Replicate the code below:

```md
$$ \class{hyphen-color}{\sum}_{n=1}^\infty a_n $$
```
14. Test your understanding by creating a block equation where each term is a different colour!

```{tip}
This is especially helpful for highlighting changes between equations or focusing attention on specific terms — great for teaching, too!
```
